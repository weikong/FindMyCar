...
import com.king.smart.car.mysmartcarapp.manager.MyOrientationListener;
import com.king.smart.car.mysmartcarapp.ui.adapter.RouteLineAdapter;
 ...
 * 同时展示如何进行节点浏览并弹出泡泡
 */
public class LocalRouteActivity extends Activity implements BaiduMap.OnMapClickListener,
    ...
    private View view_drive_line, view_transit_line, view_walk_line, view_bike_line;
    private Button btnDistance;

    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_local_route);
        Bundle bundle = this.getIntent().getExtras();
        if (bundle != null)
            bean = (LockBean) bundle.get("DATA");
        // 初始化地图
        ...
        mMapView.removeViewAt(1);//移除缩放控件
        ...
        if (bean != null && bean.getmLat() > 0 && bean.getmLon() > 0) {
            MyLocationData locData = new MyLocationData.Builder().latitude(bean.getmLat())
                    .longitude(bean.getmLon()).build();
        ...
        view_drive_line = (View) findViewById(R.id.view_drive_line);
        view_transit_line = (View) findViewById(R.id.view_transit_line);
        view_walk_line = (View) findViewById(R.id.view_walk_line);
        view_bike_line = (View) findViewById(R.id.view_bike_line);
        ImageView ivFindMe = (ImageView) findViewById(R.id.ivFindMe);
        ivFindMe.setOnClickListener(new View.OnClickListener() {
            @Override
    ...
    private void routeType(View view) {
        view_drive_line.setVisibility(View.INVISIBLE);
        view_transit_line.setVisibility(View.INVISIBLE);
        view_walk_line.setVisibility(View.INVISIBLE);
        view_bike_line.setVisibility(View.INVISIBLE);
    ...
    public void searchButtonProcess(View v) {
        ...
        if (fromNodeL == null)
            return;
        stNode = PlanNode.withLocation(fromNodeL);
        if (toNodeL != null)
            enNode = PlanNode.withLocation(toNodeL);
        else if (bean != null && !TextUtils.isEmpty(bean.getmAddress()))
            enNode = PlanNode.withCityNameAndPlaceName("成都", bean.getmAddress());
        else
            return;

        // 实际使用中请对起点终点城市进行正确的设定
        if (v == null && bean != null) {
            if (bean.getmLat() > 0 && bean.getmLon() > 0) {
                mSearch.walkingSearch((new WalkingRoutePlanOption())
                        .from(stNode).to(enNode));
                nowSearchType = 3;
                routeType(view_walk_line);
            } else {
                mSearch.drivingSearch((new DrivingRoutePlanOption())
                        .from(stNode).to(enNode));
                nowSearchType = 1;
                routeType(view_drive_line);
            }
            return;
        }
    ...
    private int calculateDistance() {
        ...
        Object step = null;
        List list = route.getAllStep();
        for (int i = 0; list != null && i < list.size(); i++) {
            step = list.get(i);
            if (step instanceof WalkingRouteLine.WalkingStep) {
                distance += ((WalkingRouteLine.WalkingStep) step).getDistance();
            }
        }
        if (distance > 0) {
            double dis = distance;
            String unit = "米";
            if (distance < 1000) {
                btnDistance.setText(distance + " " + unit);
            } else {
                dis = dis / 1000;
                unit = "公里";
                btnDistance.setText(dis + " " + unit);
            }
        }
        return distance;
    }
    ...
    public void nodeClick(View v) {
        ...
        // show popup
        popupText = new TextView(LocalRouteActivity.this);
    ...
    public void onGetWalkingRouteResult(WalkingRouteResult result) {
        if (result == null || result.error != SearchResult.ERRORNO.NO_ERROR) {
            Toast.makeText(LocalRouteActivity.this, "抱歉，未找到结果", Toast.LENGTH_SHORT).show();
        }
        ...
        if (result.error == SearchResult.ERRORNO.NO_ERROR) {
            ...
            if (result.getRouteLines().size() > 1) {
                ...
                MyTransitDlg myTransitDlg = new MyTransitDlg(LocalRouteActivity.this,
...
//        mLocClient.stop();
    ...
    private void processLocalData(BDLocation location) {
        ...
        mBaidumap.setMyLocationData(locData);
        fromNodeL = null;
        fromNodeL = new LatLng(location.getLatitude(), location.getLongitude());
        if (isFirstLoc) {
            isFirstLoc = false;
            // 修改为自定义marker
            ...
            builder.target(ll).rotate(0.0f).zoom(19.0f);
            ...
            if (bean != null) {
                if (bean.getmLat() != 0 && bean.getmLon() != 0) {
                    toNodeL = null;
                    toNodeL = new LatLng(bean.getmLat(), bean.getmLon());
                }
                searchButtonProcess(null);
            }
        }
        if (mBaidumap.getLocationConfigeration().locationMode.ordinal() != LocationMode.NORMAL.ordinal()) {
